...
class Lora:
    ...
    def initialize_LoRaWAN(self, retry_limit=3):
        ...
        self.non_blocking_delay(4)  # Delay for Arduino boot up
        ...
        for attempt in range(retry_limit):
            ...
            if bol.decode().strip() == "#JOINED":
                ...
                self.non_blocking_delay(20)
                return True
            
            self.non_blocking_delay(5)  # Delay between retries
    ...
    def passive_listen(self, timeout=15):
        ...
        while True:
            ...
            self.non_blocking_delay(0.1)  # Short delay to prevent busy waiting
    ...
    def save_message_to_csv(self, input_str, csv_filename="messages.csv"):
        if "#" in input_str and len(input_str.split("#")) == 2:
            ...
            if address in df.columns:
                df.loc[df[address].last_valid_index() + 1 if pd.notna(df[address]).any() else 0, address] = message
            else:
                df[address] = None
    ...
    def request_weather_time(self):
        ...
        if self.connected_bool:
            ...
            df.rename(columns={"m": "max temp", "n": "min temp", "p": "precipitation"}, inplace=True)
            current_date = datetime.now()
            df["day"] = [(current_date + timedelta(days=i)).strftime("%A") for i in range(len(df))]
    ...
    def send_text(self, address, text):
        ...
        if self.connected_bool:
            ...
            received_message = self.passive_listen()
            #self.non_blocking_delay(15)  # Replaces time.sleep(15)
            #self.active_listen()
    ...
    def active_listen(self, timeout=10):
        while True:
            ...
            start_time = time.time()
            while True:
                self.non_blocking_delay(1)  # Replaces time.sleep(1)
                if (time.time() - start_time) > timeout:
